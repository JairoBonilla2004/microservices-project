...
class ReactivateModuleServiceTest {
    ...
    @InjectMocks
    private ReactivateModuleService reactivateModuleService;

    private final UUID moduleId = UUID.randomUUID();

    @Test
    void should_reactivateModule_when_moduleExists() {
        Module module = new Module("Test Module", "Description", "icon-star", 1);
        module.setId(moduleId);
        module.setEstado(ec.edu.espe.master_gateway.shared.infrastructure.persistence.EstadoRegistro.INACTIVO);
        when(moduleRepositoryPort.findById(moduleId)).thenReturn(Optional.of(module));

        reactivateModuleService.execute(moduleId);
